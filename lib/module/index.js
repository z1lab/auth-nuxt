...
import { merge, uniq, isEmpty } from 'lodash'
...
function validateOptions (options) {
  if (!options.cookie || isEmpty(options.cookie)) logger.fatal('Cookie must be an object and cannot be empty, fill in the required settings fields.')

  // Enforce vuex store because auth depends on it
  if (!this.options.store) logger.fatal('Enable vuex store by creating `store/index.js`.')
}
...
function processStrategies (options) {
  ...
  for (const name in options.strategies) {
    if (!options.strategies[name]) continue

    // Clone strategy
    const strategy = Object.assign({}, options.strategies[name])

    // Guess _name property
    if (!strategy._name) strategy._name = name

    // Guess _provider property
    if (!strategy._provider) strategy._provider = strategy._name
