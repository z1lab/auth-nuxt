...
export default class OpenidScheme {
  ...
  async setTokens (result, remember = true) {
    ...
    if (remember && (remember !== 'undefined')) {
      await this.$auth.setRefreshToken(this.name, result.refresh_token, {
        expires: new Date(new Date() * 1 + result.expires_in * 2000)
      })
    }
  ...
  async login (params) {
    ...
    await this.$auth.request(this.options.token_endpoint, '$post', opts).then(async response => {
      ...
      const decoded_idToken = JSON.parse(jwt(await response.id_token).user)
